...
> เพิ่มเป็นภาพประกอบเสริมจากผังข้อความด้านบน — ไม่เปลี่ยนเนื้อหาสเปคเดิม เพื่อให้เห็นลำดับ Scene และกฎ FRAME END ชัดเจนขึ้น

```mermaid
flowchart TD
    IN["INPUT<br/>Location / GPS<br/>TARGET END FRAME = รูปที่ดินจริง"]
    S1["SCENE 1<br/>Satellite / Map<br/>+ Location Pin"]
    S2["SCENE 2<br/>Extreme Fast Zoom<br/>สู่ตำแหน่ง Pin"]
    S3["SCENE 3<br/>Cloud Transition<br/>ผ่านชั้นเมฆ (ไม่ Hard Cut)"]
    S4["SCENE 4<br/>Approach Target Image<br/>Match Angle / Ground / Lighting"]
    FT["FINAL TRANSITION<br/>Blend เข้าสู่ Target Image"]
    FF["FINAL 0.5–1.0 วิ<br/>EXACT TARGET IMAGE<br/>FREEZE / HOLD"]
    OUT["OUTPUT<br/>WF1 VIDEO<br/>+ EXACT ORIGINAL IMAGE"]
    WF2["FRAME START ของ WF2"]

    IN --> S1 --> S2 --> S3 --> S4 --> FT --> FF --> OUT
    FF -.->|"FRAME END ของ WF1 = FRAME START ของ WF2"| WF2

    style FF fill:#ffe066,stroke:#333,stroke-width:2px
    style WF2 fill:#e0e0e0,stroke:#333,stroke-dasharray: 4 2
```
