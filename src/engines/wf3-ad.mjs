// WF3 -- Original Land Photo -> Advertisement Layout.
// WF-REALESTATE-3WF-SPEC.md, third and final workflow.
//
// Takes the ORIGINAL LAND PHOTO (the same file WF1 opens and closes on) as its hero image,
// NOT WF2's finished-house frame -- spec owner's explicit correction, wf/WF3.md
// (2026-09-10): "จะขายรูปต้นฉบับ ไม่ได้ขายรูป AI" ("sell the real photo, not the AI photo").
// WF2's house is a concept visualization that plays inside WF2's own clip; it was never
// meant to be the thing the advertisement sells. `house_image_file` below still just means
// "whatever image this call was handed" -- callers now pass the land photo, not the house
// frame (see runWfFinish in pipeline.mjs, which picks the hero). Turns it into the sell
// sheet: a slow cinematic pull-back on the property with the advertisement panel rising
// into place over it.
//
