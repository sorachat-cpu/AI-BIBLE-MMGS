 ...
 *
 * Buffers are concatenated rather than appended to a string. Decoding each chunk on its own
 * splits any multi-byte character that straddles a chunk boundary into two replacement
 * characters -- which silently corrupts Thai text, and breaks LINE's webhook signature
 * outright, since the HMAC is computed over the bytes LINE actually sent. Byte length is
 * also the only thing MAX_BODY_BYTES can honestly be measured in.
 */
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && req.url === "/api/line/webhook") {
      // Raw bytes: the signature is over what LINE sent, not over a re-encoding of it.
    ...
    if (req.method === "POST" && req.url === "/api/listings/rehost-photos") {
      try {
        // A `limit` keeps one call inside a proxy's patience; progress is flushed as it
        // goes either way, so a caller can just POST again to continue.
        const p = JSON.parse((await readBody(req)) || "{}");
        const limit = Number.isFinite(Number(p.limit)) && Number(p.limit) > 0
          ? Number(p.limit)
          : undefined;
        sendJson(res, 200, { ok: true, data: await backfillPhotos(limit ? { limit } : {}) });
      } catch (err) {
