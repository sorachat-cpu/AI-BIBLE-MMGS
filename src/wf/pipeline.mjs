...
export async function runWfFinish(input, options = {}) {
  ...
  // WF3's hero is the ORIGINAL LAND PHOTO, not WF2's AI-generated house -- spec owner's
  // explicit correction (wf/WF3.md, 2026-09-10): "จะขายรูปต้นฉบับ ไม่ได้ขายรูป AI". The house
  // WF2 builds is a concept visualization that plays inside WF2's own clip; it is not what
  // WF3 opens on or sells. Preference order: WF1's frozen final frame (the exact same file
  // as the real land photo, guaranteed identical by finishWf1) -> the raw land_image input
  // when WF1 wasn't run this call -> WF2's house frame only as a last resort, so a caller
  // that supplies nothing else (WF2-only runs, old callers) does not silently get no ad at all.
  const hero = results.wf1?.final_frame ?? land_image ?? house_image ?? results.wf2?.final_frame;
  if (!skip_wf3 && hero) {
